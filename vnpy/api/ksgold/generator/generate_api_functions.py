...
class ApiGenerator:
    ...
    def process_line(self, line: str):
        ...
        if "virtual void On" in line:
            ...
            self.process_function_special(line)

    def process_function_special(self, line):
        words = line.split(" ")
        cpp_type = words[1]
        name = words[2].split("(")[0]

        name = f"{cpp_type} {name}"
        ...
        with open(filename, "w") as f:
            for name, values in self.functions.items():
                if name.startswith("Req"):
                    ...
                    if "RegisterSpi" in name:
                        line = "void createGoldQutoApi(string pszFlowPath = \" \");\n\n"
                        f.write(line)
                    else:
                        ...
                        if not values:
                            ...
                            content = ""
                            for name_, type_ in values.items():
                                if type_ == "char":
                                    type_ = "string"

                                if "[" in name_:
                                    name_ = name_.split("[")[0]
                                elif name_.startswith("n"):
                                    if name_ == "nCount":
                                        continue
                                    else:
                                        name_ = "reqid"
                        ...
                        line = f"{cpp_type} {new_name}({content});\n\n"
                        self.spc_req[new_name] = content
        ...
        with open(filename, "w") as f:
            for name, d in self.functions.items():
                if name.startswith("Req"):
                    ...
                    if "RegisterSpi" in name:
                        f.write("void createGoldQutoApi(string pszFlowPath)\n")
                        f.write("{\n")
                        f.write("\tthis->api = CKSGoldQuotApi::CreateGoldQutoApi(pszFlowPath.c_str());\n")
                        f.write(f"\tthis->api->RegisterSpi(this);\n")
                        f.write("};\n\n")

                    else:
                        spc_name = name[0].lower() + name[1:]
                        print(spc_name)
                        content = self.spc_req[spc_name]

                        if spc_name == "init":
                            f.write(
                                f"{cpp_type} {self.class_name}::{spc_name}({content})\n")
                            f.write("{\n")
                            f.write(f"\tthis->active = true;\n")
                            f.write(f"\tthis->task_thread = thread(&MdApi::processTask, this);\n")
                            f.write(f"\tthis->api->Init();\n")
                            f.write("};\n\n")
                        else:
                            ...
                            if cpp_type == "void":
                                f.write(f"\tthis->api->{name}({content})\n")
                            else:
                                f.write(f"\t{cpp_type} i =this->api->{name}({content})\n")
                                f.write("\treturn i;\n")
                            f.write("};\n\n")
...
if __name__ == "__main__":
    md_generator = ApiGenerator("../include/ksgold/KSGoldQuotMarketDataApi.h", "ksgold", "md", "MdApi")
    md_generator.run()

    td_generator = ApiGenerator("../include/ksgold/KSGoldTraderApi.h", "ksgold", "td", "TdApi")
    td_generator.run()
